...
├── data/
│   ├── fca/                  # Scraped source (gitignored, re-scraped); derived chunks gitignored
...
**Data in git:** ground-truth (`qa/`) and prompts are pinned for reproducibility. The scraped source corpus (`fca/fca_handbook.jsonl`, ~31MB) is **gitignored**, not pinned — it's regenerated by running `scripts/scrape_fca.py`, which is the source of truth. Everything else derived (section markdown, chunk prefixes) is also gitignored and rebuilt by the scripts.
